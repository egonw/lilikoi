...
MetaTOpathway <- function(
    q.type, hmdb = T, pubchem = T, chebi = F, kegg = T, metlin = F) {
...
sanity_check <- function(name.map, hit.inx) {
  ...
  if (length(todo.inx) / length(name.map$hit.inx) > 0.5) {
    print(
        "Over half of the compound IDs could not be matched to our database. Please make
                            sure that correct compound IDs or common compound names are used.")
  } else if (length(todo.inx) > 100) {
    print(
        "There are >100 compounds without matches. You can either proceed or if necessary,
    ...
    print(paste0((length(name.map$hit.inx) - length(todo.inx)), " ", "out of ",
                 length(name.map$hit.inx), " ", "Matched metabolites", " ",
                 round((length(name.map$hit.inx) /
                        (length(name.map$hit.inx) + length(todo.inx))), 1) *
                 100, " ", "%"))
    cat("\n")
    print(paste0((length(todo.inx)), " ", "out of ", length(name.map$hit.inx),
                 " ", "UnMatched metabolites", " ",
                 round((length(todo.inx) /
                        (length(name.map$hit.inx) + length(todo.inx))), 1) *
                 100, " ", "%"))
...
NameMappingExact <- function() {
  ...
  hit.inx = vector(mode = 'numeric', length = length(qvec));  # record hit
                                                              # index, initial 0
  match.values =
      vector(mode = 'character', length = length(qvec));  # the best matched
                                                          # values (hit names),
                                                          # initial ""
  match.state =
      vector(mode = 'numeric', length = length(qvec));  # match status - 0, no
                                                        # match; 1, exact match;
                                                        # initial 0
...
GetMappingResultTable <- function() {
  ...
  if (dataSet$q.type == "name") {
    no.prestyle <-
        "<strong style=\"background-color:yellow; font-size=125%; color=\"black\">"
    no.poststyle <- "</strong>"
  } else {
    no.prestyle <-
        "<strong style=\"background-color:red; font-size=125%; color=\"black\">"
    no.poststyle <- "</strong>"
  }
  ...
  colnames(csv.res) <- c(
      "Query", "Match", "HMDB", "PubChem", "ChEBI", "KEGG", "METLIN", "Comment")
  ...
  for (i in 1:length(qvec)) {
    ...
    html.res[i, ] <- c(
        paste(pre.style, qvec[i], post.style, sep = ""),
        paste(ifelse(match.state[i] == 0, "", hit.values[i]), sep = ""),
        paste(
            ifelse(match.state[i] == 0 ||
                   is.na(hit$hmdb_id) ||
                   hit$hmdb_id == "" || hit$hmdb_id == "NA", "-",
                   paste("<a href=http://www.hmdb.ca/metabolites/", hit$hmdb_id,
                         " target='_blank'>", hit$hmdb_id, "</a>", sep = "")),
            sep = ""),
        paste(
            ifelse(
                match.state[i] == 0 ||
                is.na(hit$pubchem_id) ||
                hit$pubchem_id == "" || hit$pubchem_id == "NA", "-",
                paste(
                    "<a href=http://pubchem.ncbi.nlm.nih.gov/summary/summary.cgi?cid=",
                    hit$pubchem_id, " target='_blank'>", hit$pubchem_id, "</a>",
                    sep = "")), sep = ""),
        paste(
            ifelse(
                match.state[i] == 0 ||
                is.na(hit$chebi_id) ||
                hit$chebi_id == "" || hit$chebi_id == "NA", "-",
                paste("<a href=http://www.ebi.ac.uk/chebi/searchId.do?chebiId=",
                      hit$chebi_id, " target='_blank'>", hit$chebi_id, "</a>",
                      sep = "")), sep = ""),
        paste(ifelse(match.state[i] == 0 ||
                     is.na(hit$kegg_id) ||
                     hit$kegg_id == "" || hit$kegg_id == "NA", "-",
                     paste("<a href=http://www.genome.jp/dbget-bin/www_bget?",
                           hit$kegg_id, " target='_blank'>", hit$kegg_id,
                           "</a>", sep = "")), sep = ""),
        paste(
            ifelse(
                match.state[i] == 0 ||
                is.na(hit$metlin_id) ||
                hit$metlin_id == "" || hit$metlin_id == "NA", "-",
                paste(
                    "<a href=http://metlin.scripps.edu/metabo_info.php?molid=",
                    hit$metlin_id, " target='_blank'>", hit$metlin_id, "</a>",
                    sep = "")), sep = ""),
        ifelse(match.state[i] != 1, "View", ""))
    csv.res[i, ] <- c(
        qvec[i], ifelse(match.state[i] == 0, "NA", hit.values[i]),
        ...
        ifelse(match.state[i] == 0, "NA", hit$metlin_id), match.state[i])
  }
...
PerformApproxMatch <- function(inx) {
  ...
  for (j in s) {
    new.q <- q
    if (q.length > 32) {  # note: agrep fail for exact match when length over 32
                          # characters
      new.q <- substr(q, 1, 32)
    ...
    matched <- FALSE
    matched.inx <- agrep(
        q, syns.vec, ignore.case = T, max.distance = j, useBytes = T)
    ...
    if (length(matched.inx) > 0) {
      ...
      for (n in 1:length(matched.inx)) {
        ...
        hit3.inx <- agrep(
            q, nm.vec, ignore.case = T, max.distance = j, useBytes = T)
        ...
        if (length(hit3.inx) > 0) {
          ...
          for (k in 1:length(hit3.inx)) {
            ...
            hit3.score[k] <- j +
                abs(nchar(nm.vec[idx]) - nchar(q)) / (10 * nchar(q))
          }
      ...
      rm.inx <- is.na(candidates[, 2]) |
          candidates[, 2] == "NA" | candidates[, 2] == ""
      candidates <- candidates[!rm.inx, ]
      candidates <- candidates[
          order(candidates[, 3], decreasing = F), , drop = F]
...
SetCandidate <- function(query_inx, can_inx) {
  ...
  if (can_inx <= nrow(can_mat)) {
    ...
    if (ncol(csv.res) > 6) {  # general utilities
      csv.res[query_inx, ] <- c(
          csv.res[query_inx, 1], name.map$hit.values[query_inx], hit$hmdb_id,
          hit$pubchem_id, hit$chebi_id, hit$kegg_id, hit$metlin_id, 1)
    } else {
      csv.res[query_inx, ] <- c(
          csv.res[query_inx, 1], name.map$hit.values[query_inx], hit$hmdb_id,
          hit$pubchem_id, hit$kegg_id, 1)
    }
    write.csv(csv.res, file = "name_map.csv", row.names = F)
    dataSet$map.table <<- csv.res
