PDSfun <- function(qvec) {
  ...
  #' meta_path.RData
  #'
  #' @param qvec This is the Metabolite_pathway_table from MetaTOpathway function. This table includes the metabolite ids and their corresponding HMDB ids
  #' @keywords PDS
  #' @export
  ...
  PDS <- quantify_pathways_deregulation(
      as.matrix(newData), row.names(newData), metabolites.list, pathway.list,
      as.logical(phe), attempts = 5, min_exp = 0, min_std = 0)

  qpdmat <<- matrix(
      as.data.frame(PDS$scores), nrow = length(names(PDS$scores)), byrow = TRUE)
