...
machine_learning <- function(PDSmatrix, selected_Pathways_Weka) {
  ...
  #'
  #'
  prostate_df = data.frame(t((PDSmatrix[selected_Pathways_Weka, ])),
                           Label = Metadata$Label, check.names = T)
  ...
  trainIndex <- createDataPartition(
      prostate_df$subtype, p = .8, list = FALSE, times = 1)
  ...
  set.seed(7)  # This ensures that the same resampling sets are used,which will
               # come in handy when we compare the resampling profiles between
               # models.
  ...
  garbage <- capture.output(
      fit.cart <- train(subtype ~ ., data = irisTrain, method = 'rpart',
  ...
  performance_training[2, 1] =
      fit.cart$results$Sens[which.max(fit.cart$results$ROC)]  # sen
  performance_training[3, 1] =
      fit.cart$results$Spec[which.max(fit.cart$results$ROC)]  # spec
  ...
  cart.ROC <- roc(predictor = cartClasses$Normal, response = irisTest$subtype,
  ...
  garbage <- suppressWarnings(
      capture.output(fit.lda <- train(subtype ~ ., data = irisTrain,
                                      method = 'lda', trControl = control,
                                      metric = "ROC", trace = F)))  # loclda)
  ...
  performance_training[2, 2] =
      fit.lda$results$Sens[which.max(fit.lda$results$ROC)]  # sen
  performance_training[3, 2] =
      fit.lda$results$Spec[which.max(fit.lda$results$ROC)]  # spec
  ...
  lda.ROC <- roc(predictor = ldaClasses$Normal, response = irisTest$subtype,
                 levels = rev(levels(irisTest$subtype)))
  ...
  set.seed(7)
  garbage <- capture.output(
      fit.svm <- train(subtype ~ ., data = irisTrain, method = "svmRadial",
                       trControl = control, metric = "ROC"))
  ...
  svm.ROC <- roc(predictor = svmClasses$Normal, response = irisTest$subtype,
                 levels = rev(levels(irisTest$subtype)))
  ...
  set.seed(7)
  garbage <- capture.output(
      fit.rf <- train(subtype ~ ., data = irisTrain, method = "rf",
                      trControl = control, metric = "ROC"))
  ...
  rf.ROC <- roc(predictor = rfClasses$Normal, response = irisTest$subtype,
                levels = rev(levels(irisTest$subtype)))
  ...
  garbage <- suppressWarnings(
      capture.output(
          fit.gbm <- train(subtype ~ ., data = irisTrain, method = "gbm",
                           trControl = control, metric = "ROC")))
  ...
  performance_training[2, 5] =
      fit.gbm$results$Sens[which.max(fit.gbm$results$ROC)]  # sen
  performance_training[3, 5] =
      fit.gbm$results$Spec[which.max(fit.gbm$results$ROC)]  # spec
  ...
  gbm.ROC <- roc(predictor = gbmClasses$Normal, response = irisTest$subtype,
                 levels = rev(levels(irisTest$subtype)))
  ...
  set.seed(7)
  garbage <- capture.output(
      fit.pam <- train(subtype ~ ., data = irisTrain, method = "pam",
  ...
  performance_training[2, 6] =
      fit.pam$results$Sens[which.max(fit.pam$results$ROC)]  # sen
  performance_training[3, 6] =
      fit.pam$results$Spec[which.max(fit.pam$results$ROC)]  # spec
  ...
  pam.ROC <- roc(predictor = pamClasses$Normal, response = irisTest$subtype,
                 levels = rev(levels(irisTest$subtype)))
  ...
  garbage <- suppressWarnings(
      capture.output(
          fit.log <- train(subtype ~ ., data = irisTrain, method = "glmnet",
                           trControl = control, metric = "ROC")))
  ...
  performance_training[2, 7] =
      fit.log$results$Sens[which.max(fit.log$results$ROC)]  # sen
  performance_training[3, 7] =
      fit.log$results$Spec[which.max(fit.log$results$ROC)]  # spec
  ...
  log.ROC <- roc(predictor = logClasses$Normal, response = irisTest$subtype,
                 levels = rev(levels(irisTest$subtype)))
  ...
  plot(smooth(log.ROC, method = smooth_method), col = "lightgoldenrod2",
       main = "Testing ROC", cex.lab = 1.5)
  #plot(log.ROC,col="lightgoldenrod2",main="Testing ROC",print.auc=T)

  legend(0.2, 0.4, legend = c('RPART', 'LDA', 'SVM', 'RF', 'GBM', 'PAM', 'LOG'),
         col = c("red", "green", "black", "orange", "blue", "hotpink",
                 "lightgoldenrod2"), lty = 1:2, cex = 1)
  # dev.off()
  ...
  output8 <- do.call(
      rbind, lapply(Balanced_accuracy_test, matrix, ncol = 7, byrow = TRUE))
  ...
  Balanced_accuracy =
      data.frame(Balanced_accuracy = t(t(Balanced_accuracy_test_mean)))
  ...
  performance_data =
      data.frame(AUC = AUC, SENS = SENS, SPEC = SPEC, trainingORtesting,
                 Algorithm = (rep(t(c('RPART', 'LDA', 'SVM', 'RF', 'GBM', 'PAM',
                                      'LOG')), 2)))

  performance_data_test =
      data.frame(AUC = data.frame(AUC = t((t(AUC_test_mean)))),
                 SENS = data.frame(SENS = t((t(SENS_test_mean)))),
                 SPEC = data.frame(SPEC = t((t(SPEC_test_mean)))), F1 = F1,
                 Balanced_accuracy = Balanced_accuracy, testing_only,
                 Algorithm = (rep(t(c('RPART', 'LDA', 'SVM', 'RF', 'GBM', 'PAM',
                                      'LOG')), 1)))
  ...
  p1 = ggplot(data = melted_performance_data[trainingORtesting == 'training', ],
  ...
  p2 = ggplot(data = melted_performance_data[trainingORtesting == 'testing', ],
              aes(x = Algorithm, y = value, fill = variable)) +
       geom_bar(stat = "identity", position = position_dodge()) + xlab("") +
       ylab("") + ggtitle("Testing") +
       theme(plot.title = element_text(hjust = 0.5),
             axis.text = element_text(size = 15, face = "bold"),
             axis.title = element_text(size = 14, face = "bold")) +
       labs(fill = "")
  ...
  p3 = ggplot(data = melted_performance_data_test,
              aes(x = Algorithm, y = value, fill = variable)) +
       geom_bar(stat = "identity", position = position_dodge()) +
